...
# Predicciones
preds = {name: f(X_test) for name, (_, f) in models.items()}
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([dcc.Graph(id='featimp-graph')],
                 style={'width': '48%', 'display': 'inline-block'})
    ], style={'padding': '10px 25px'}),
...
def update_all(start_date, end_date, model_name, show_checks):
    ...
    if 'real' in show_checks:
        ts_fig.add_trace(go.Scatter(x=df_f['fecha'], y=df_f['consumo_suba_lpcd'],
    ...
    if 'roll3' in show_checks:
        ...
        ts_fig.add_trace(go.Scatter(x=df_f['fecha'], y=df_f['consumo_roll_3_local'],
                                    mode='lines', name='Consumo 3m (rolling)'))
    if 'pred' in show_checks:
        ts_fig.add_trace(go.Scatter(x=df_pred_show['fecha'], y=df_pred_show['predicted'],
                                    mode='lines+markers', name=f'Predicción ({model_name})'))

    ts_fig.update_layout(title="Serie de Consumo (real vs predicción)",
                         xaxis_title="Fecha", yaxis_title="Consumo (lpcd)")

    scatter_fig = px.scatter(df_f, x='nivel_chingaza_pct', y='consumo_suba_lpcd',
                             color=df_f['consumo_suba_lpcd'],
                             labels={'nivel_chingaza_pct': 'Nivel embalse (%)',
                                     'consumo_suba_lpcd': 'Consumo (lpcd)'},
                             title="Nivel embalse vs Consumo (puntos reales)")

    feat_fig = px.bar(feat_imp, x='importance', y='feature', orientation='h',
                      title='Importancia de features (Random Forest)')
    feat_fig.update_layout(yaxis={'categoryorder': 'total ascending'})

    m = calc_metrics(df_pred_show['consumo_suba_lpcd'],
                     df_pred_show['predicted']) if len(df_pred_show) > 0 else metrics_summary[model_name]
    cards = [
        html.Div([html.H4("R²"), html.P(f"{m['r2']:.3f}")],
                 style={'padding': '10px', 'border': '1px solid #eee', 'borderRadius': '6px',
                        'width': '100px', 'textAlign': 'center'}),
        html.Div([html.H4("MAE"), html.P(f"{m['mae']:.2f}")],
                 style={'padding': '10px', 'border': '1px solid #eee', 'borderRadius': '6px',
                        'width': '100px', 'textAlign': 'center'}),
        html.Div([html.H4("RMSE"), html.P(f"{m['rmse']:.2f}")],
                 style={'padding': '10px', 'border': '1px solid #eee', 'borderRadius': '6px',
                        'width': '100px', 'textAlign': 'center'}),
    ]
